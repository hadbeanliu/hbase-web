...
//初始化地图弹出框
$('.map-pin-box .pic-map,.map-annotation .annot2').click(function(){
	$('.map-piker-show').slideToggle(100);
})
$('.map-pin-box input').click(function(){
	$('.map-piker-show').show();
});
...
$(document).bind('click',function(){
	$('.map-piker-show').css('display','none');
	$('.result-box').css('display','none');
});
$('.map-pin-box').bind('click',function(e){
	stopPropagation(e);
});
